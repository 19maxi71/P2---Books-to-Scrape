...
# Fo mettre les liens des catégories principales dans un cycle qui parcours chaque catégorie
...
def scrape_category_links(category_url, category_name):
    ...
    if soup.find('li', class_='current'):
        ...
        x = int(page_number[0].text.split()[-1])
        for i in range(1, x+1):
            category_url = f"{url_home}/category/books/{category_name}/page-{i}.html"
            # cycle pour parcourir les pages de la catégorie
            page = requests.get(category_url)
            soup = BeautifulSoup(page.text, "html.parser")
            links = soup.find_all('h3')  # cherche les links dans les balises h3
            links_book_urls = [urljoin(category_url, link.find('a')['href']) for link in links]
        ...
        category_url = f"{url_home}/category/books/{category_name}/index.html"
        page = requests.get(category_url)
        soup = BeautifulSoup(page.text, "html.parser")
        links = soup.find_all('h3')  # cherche les links dans les balises h3
        links_book_urls = [urljoin(category_url, link.find('a')['href']) for link in links]
