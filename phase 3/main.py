...
def scrape_category_links(category_url):
    ...
    if not page_number: # si pas de numéro de page, alors la page est la page initial (avec index.html à la fin)
        ...
        for i in range(1, x+1):     # cycle pour parcourir les pages de la catégorie
            category_url = f"{category_url.split('/index.html')[0]}/page-{i}.html" # cycle pour parcourir les pages de la catégorie
    page = requests.get(category_url) # ici category_url est la page de la catégorie à scraper après la condition ci-dessus
    soup = BeautifulSoup(page.text, "html.parser")
...
links_cat_urls = get_cat_links() # lance la fonction get_cat_links et stocke le résultat dans links_cat_urls

for category_url in links_cat_urls: # boucle pour parcourir les links_cat_urls qui lance la fonction scrape_category_links
    result = scrape_category_links(category_url) # stocke le résultat de la fonction scrape_category_links dans result
#return(result) # retourne le résultat de la fonction scrape_category_links dans result. stock dans result les liens des livres de chaque catégorie.
# print(result)
